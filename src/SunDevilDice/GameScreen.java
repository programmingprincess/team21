...
public class GameScreen extends JPanel {
	...
	// a map of player names to their respective player seat
	...
	public GameScreen(Vector<String> playerList) {
		...
		if (playerList.size() > 0) {
			...
			playerSeat1.setBorder(new LineBorder(Color.white, 5, true));
		}
	 ...
	 * @param newScore
	 *            the value of the updated score
	 */
	...
	/**
	 * Highlights the border of the given player and rests the border of the
	 * current player, then updates current player
	 * 
	 * @param player
	 *            the player whose seat border is to be updated
	 */
	public void updatePlayerBorder(String player) {
		...
		seatDictionary.get(player).setBorder(new LineBorder(Color.white, 5, true));
		currentSeat = player;
	}
}
